...
class FileLoadWithCache {
    ...
    public function __construct($feedUrl, $urlgettimeout, $cacheEnable, $cacheFile, $cacheExpireTime, $oauth_bearer_access_key, $http_header_default_useragent_flag, $debugmode, $type){
...
//        $url .= '?_filters={"status":"400", "sort":"start desc"}';
	    if ($this->type == 'event') {
		    $product_type = 1;
	    }
	    else {
        ...
        if (get_option('jci_partner_global_id') != null) {
	        $url .= '?_filters={"status":"400", "hosts_globalid":"'.get_option('jci_partner_global_id').'","product_type":"'.$product_type.'","sort":"start desc"}';
        }
        else {
	        $url .= '?_filters={"status":"400", "sort":"start desc"}';
        }
